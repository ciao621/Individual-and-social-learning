...
function findRemeetingTimes(mAdj, P1IL)
    N = size(mAdj, 1)
    rowSums = sum(mAdj, dims=2)
    P = mAdj ./ rowSums
        ...
        for j in 1:N
            ...
            if i == j
                ...
                for k in 1:N
                    idx_jk = (j-1)*N + k
                    idx_ik = (i-1)*N + k
                    A[idx, idx_jk] -= coeff * P[i, k]
                    A[idx, idx_ik] -= coeff * P[j, k]
                end
            end
        end
    end
...
function program_c(b_all, mu, PIL, c, N, w, degree, time_all, number_structural, Calculation_q)
    ...
    steady_SL_cooperation = zeros(Length_ball, 4)
    steady_SL_defect = zeros(Length_ball, 4)


    steady_IL_cooperation = zeros(Length_ball, 8)
    steady_IL_defect = zeros(Length_ball, 8)
